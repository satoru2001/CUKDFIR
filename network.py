...
class Student_resnet(nn.Module):
    def __init__(self, block, layers, hash_size):
        super().__init__()
        ...
        self.binary = nn.Linear(2048,hash_size)
...
def student_resnet_model(hashsize):
    layers=[2, 3, 5, 3, 2]
    model = Student_resnet(BasicBlock, layers,hashsize)
    return model
...
def student_alexnet_model(hashsize):
    layers=[2, 3, 5, 3, 2]
    model = Student_alexnet(BasicBlock, layers, hashsize)
    return model